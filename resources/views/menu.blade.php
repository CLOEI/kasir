    ...
    <div class="p-4 bg-gray-600 min-h-screen flex flex-col items-center">
        ...
        <div class="mt-8 w-full max-w-2xl">
            @foreach($products as $product)
                <div class="bg-white p-6 rounded-lg mb-4 shadow-md flex justify-between items-center">
                    <div>
                        <h2 class="text-lg font-bold">{{ $product->name }}</h2>
                        <form action="/menu/add_to_cart" method="post" class="mt-2">
                            @csrf
                            <input type="hidden" name="product_id" value="{{ $product->id }}">
                            <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-600 active:scale-95 transition-transform">Add to cart</button>
                        </form>
                    </div>
                </div>
            @endforeach
